# CAGPoster_PrairieOakPaleo
Supporting documentation for the poster "Gronding research in place: Paleoecology guided by Indigenous Fire Stewardship and ecosystem diversity" presented by Larissa Bron at the annual CAG/CARTO conference in June 2026.

## Poster PDF
.....put the link in here....

## Prairie Oak (CAMAS FOOD SYSTEMS??) Ecosystems Need Fire
......paste poster text here...

Garry oak ecosystems are culturally important kwetlal/ЌL̵O,EL/Camas (Camassia sp.) food systems, that can also be called prairie oak ecosystems, or oak-associated ecosystems, when communicating the herbaceous vegetation diversity created by Indigenous land management that augments the characteristic climate and soils of the WPG ecoregion. The Garry oak (Quercus garryana) tree is commonly found in these landscapes, though it is not the sole determinant of ecosystem presence, so there is risk that many of the cultural and ecological values are obscured when solely focusing on the tree, providing sufficient rationale for shifting the language we use to describe these landscapes. 

Physical descriptions of prairie oak ecosystems underscore that the unifying features are the presence of extensive plant diversity and prominence of low-to-mixed tree canopy cover that lend to ecosystem structural complexity. Commonly observed plant communities include prairies and meadows (0-5% canopy), savannas (5-25% canopy), and woodlands (25-75% canopy) that exist within a landscape mosaic alongside many other communities including vernal pools, rock outcrops, coastlines and dunes, wetlands, and forests. 

Many knowledge sources have identified how imperative Indigenous Fire Stewardship (IFS) is to the creation and maintenance of these complex landscapes which have existed in the ecoregion for at least thousands of years. This includes:
- Advocacy by Indigenous Peoples (e.g. Bryce, 2020)
- Current prairie oak landscapes with a high number of geophytes and other food plants, fire-adapted plants, and oak trees with open growing profiles (e.g. SOURCES)
- Ethnographic accounts of Indigenous cultures throughout the ecoregion (e.g. DECIDE WHICH REFERENCES ARE ETHNOGRAPHIC VS. ARCHEOLOGIC, Garibaldi and Turner, 2004; Turner, 2014)
- Archeologic features like camas pit cook ovens and village sites (e.g. Carney et al., 2021; Lyons and Ritchie, 2017 (?? ethno))
- Historical ecology
- Dendrochronology

IFS has been practiced globally since before knowable time, occurring whenever humans modify natural fire patterns, "fire regimes," to maintain desired ecosystem conditions (Hoffman et al., 2021; Lake and Christianson, 2019). In prairie oak landscapes, the fires associated with IFS are not homogenous over a landscape, instead IFS fire regimes exhibit complex spatial and temporal patterns resulting from achieving a multiplicity of land management targets (Clark et al., 2021; Coughlan and Petty, 2013; Long et al., 2021; Roos et al., 2019; Turner, 2014). 

#3 Improving Paleoecological Data
......paste poster text here...

## Methodological Adaptations
......paste poster text here...

## Regional Research Gaps
......paste poster text here...
......point to .csv of lit review spreadsheet...

## Reflection
......paste poster text here...

## Application
......paste poster text here...

## References
......reference list for in-text citations...

## Spatial Data
. shapefiles
. database connections (native land)
. map information
. map pdf

## Acknowledgements
. Funding
. Supervisors
. Community (people and place)

## Learn more about Prairie-Oak Ecosystems!
. SFN
. CPOP
. CCS
. GOERT
. Swan Lake


